...
function activate(context) {
    ...
    let setApiKey = vscode.commands.registerCommand('credbot.setApiKey', async () => {
        const apiKey = await vscode.window.showInputBox({
            prompt: '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',
            password: true
        });
        if (apiKey) {
    ...
    let startCommand = vscode.commands.registerCommand('credbot.start', async () => {
        ...
        const apiKey = context.globalState.get('groqApiKey');
        if (!apiKey) {
        ...
        const panel = vscode.window.createWebviewPanel('credbot', 'CredBot AI Assistant', vscode.ViewColumn.Beside, { enableScripts: true });
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            ...
            try {
                ...
                const response = await axios_1.default.post(groqApiUrl, {
